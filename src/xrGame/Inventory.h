...
public:
	...
	// Fake cell inventory functions, replacement for UIActorInventory funcs
	bool					ActorCanTakeItem	(CInventoryItem *inventory_item) const;
	bool                    CanPutInPockets		(PIItem pIItem);
	bool                    CanPutInPockets		(PIItem pIItem, const Ivector2& pos);
	bool                    CanSwap				(PIItem sender, PIItem recipient);
	void                    Swap				(PIItem sender, PIItem recipient);
